...
import initializeFirebaseServer from "../../../util/firebaseServer";
...
export default async function login(
  ...
  const { address, error } = await verifyLogin(
    process.env.NEXT_PUBLIC_AUTH_DOMAIN as string,
    payload
  );
  if (!address) {
